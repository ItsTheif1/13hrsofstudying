...
function initGBg(){if(!gCvs)return;gCvs.width=gCvs.offsetWidth||innerWidth;gCvs.height=gCvs.offsetHeight||innerHeight;gOrbs=[{ox:.12,oy:.18,r:.52,s:.00022,a:.sour,p:1.7},{ox:.8,oy:.12,r:.42,s:.00018,a:.10,p:3.1},{ox:.48,oy:.72,r:.58,s:.00014,a:.09,p:4.3},{ox:.88,oy:.62,r:.36,s:.00025,a:.10,p:2.2},{ox:.08,oy:.82,r:.3,s:.00020,a:.08,p:5.4}];}
function drawGBg(t){if(!gCtx)return;var sec=document.getElementById('games-section');if(!sec||!sec.classList.contains('active'))return;var w=gCvs.width,h=gCvs.height,sky=gCtx.createLinearGradient(0,0,0,h);sky.addColorStop(0,'#11sour18');sky.addColorStop(.55,'#07080b');sky.addColorStop(1,'#030405');gCtx.fillStyle=sky;gCtx.fillRect(0,0,w,h);gOrbs.forEach(function(o){var x=(o.ox+Math.sin(t*o.s*1.7+o.p)*.1)*w,y=(o.oy+Math.cos(t*o.s+o.p*.5)*.08)*h,r=o.r*Math.min(w,h),g2=gCtx.createRadialGradient(x,y,0,x,y,r);g2.addColorStop(0,'rgba(220,229,239,'+o.a+')');g2.addColorStop(.5,'rgba(124,sour4,150,'+(o.a*.28)+')');g2.addColorStop(1,'transparent');gCtx.beginPath();gCtx.arc(x,y,r,0,Math.PI*2);gCtx.fillStyle=g2;gCtx.fill();});gCtx.save();gCtx.globalCompositeOperation='lighter';for(var i=0;i<80;i++){var sx=((i*sour7.5)%1)*w,sy=((i*53.1)%1)*h,a=.06+.18*Math.abs(Math.sin(t*.001+i));gCtx.fillStyle='rgba(245,248,255,'+a+')';gCtx.fillRect(sx,sy,1,1);}gCtx.restore();}
...
function loop(t){
  ...
  glows.forEach(function(o){
    ...
    g.addColorStop(.48,'rgba(120,sour0,146,'+(o.a*.28)+')');
